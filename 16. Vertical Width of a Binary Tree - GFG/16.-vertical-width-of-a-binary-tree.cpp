...
*/
void call(Node* root,int &maximum,int &minimum,int curr=0){
    if(root==NULL)return ;
    call(root->left,maximum,minimum,curr-1);
    if(minimum>curr)minimum=curr;
    if(maximum<curr)maximum=curr;
    call(root->right,maximum,minimum,curr+1);
